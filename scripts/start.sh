...
count=${1:-3} # default to 3 if unset
count="$(($count-1))" # To account for the leader

# Defaults to a count of 2 to create a 3 node cluster, including the "leader"
docker-compose up --build --detach --scale consul-workers=${count}   # --scale vault-workers=$(or $(count), 2)
